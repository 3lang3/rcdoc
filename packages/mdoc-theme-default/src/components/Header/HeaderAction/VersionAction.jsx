...
import { MdocSiteContext, Dropdown } from '@mdoc/theme';
...
export default () => {
  ...
  const overlay = React.useMemo(() => {
    ...
    return config?.site?.versions.map(ver => (
      <NavbarLink className="doc-navbar__sublink" key={ver.title} path={ver.path}>{ver.title}</NavbarLink>
    ));
  ...
  return (
    <Dropdown
      overlay={overlay}
      className="doc-navbar__item doc-header-action__lang"
    >
      v{packageVersion}
    </Dropdown>
  );
