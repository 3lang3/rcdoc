...
export interface Options {
  ...
  /**
   * Enable resolve node_module file
   */
...
export default function viteReact(opts: Options = {}): PluginOption[] {
  ...
  const resolveNodeModules = opts.resolveNodeModules ?? false;
  ...
  const viteBabel: Plugin = {
    ...
    async transform(code, id, options) {
      ...
      if (/\.(mjs|[tj]sx?)$/.test(extension)) {
        ...
        const isNodeModules = !resolveNodeModules && id.includes('/node_modules/');
        const isProjectFile =
        ...
        if (!isProjectFile || isJSX) {
          if (useAutomaticRuntime) {
            ...
            const [restoredAst, isCommonJS] =
              !isProjectFile && !isJSX ? await restoreJSX(babel, code, id) : [null, false];
        ...
        const shouldSkip =
          !plugins.length && !babelOptions.configFile && !(isProjectFile && babelOptions.babelrc);
